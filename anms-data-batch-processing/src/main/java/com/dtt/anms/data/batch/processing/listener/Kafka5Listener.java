...
public class Kafka5Listener {
    ...
    @Value(value = "${anms.task.kafka.kafka5.configId}")
    int configId;
    @Value(value = "${anms.task.kafka.kafka5.sleepTime}")
    int sleepTimp;

    @KafkaListener(topics = "#{'${anms.task.kafka.kafka5.topics}'.split(',')}", groupId = "${anms.task.kafka.kafka5.groupId}", autoStartup = "${anms.task.kafka.kafka5.isStart}")
    public void receive(ConsumerRecords<String, String> consumerRecords, Acknowledgment ack) {
        try {
           if (ObjectUtil.isNull(kafkaBatchClick) || ObjectUtil.isEmpty(kafkaBatchClick)) {
                ...
                log.info("kafka5-首次初始化数据库配置:" + kafkaBatchClick.toString());
            }
