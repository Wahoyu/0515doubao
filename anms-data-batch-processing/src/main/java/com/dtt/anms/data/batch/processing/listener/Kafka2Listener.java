...
public class Kafka2Listener {
    ...
    @Value(value = "${anms.task.kafka.kafka2.configId}")
    int configId;
    @Value(value = "${anms.task.kafka.kafka2.sleepTime}")
    int sleepTimp;

    @KafkaListener(topics = "#{'${anms.task.kafka.kafka2.topics}'.split(',')}", groupId = "${anms.task.kafka.kafka2.groupId}", autoStartup = "${anms.task.kafka.kafka2.isStart}")
    public void receive(ConsumerRecords<String, String> consumerRecords, Acknowledgment ack) {
        try {
            if (ObjectUtil.isNull(kafkaBatchClick) || ObjectUtil.isEmpty(kafkaBatchClick)) {
                ...
                log.info("kafka2-首次初始化数据库配置:" + kafkaBatchClick.toString());
            }
            log.info("topic:{},收到的数据量为:{}",kafkaBatchClick.getKafkaTopicName(),consumerRecords.count());
            log.info("consumerRecords-kafka2:"+consumerRecords);
