...
public class Kafka1Listener {
    ...
    @Value(value = "${anms.task.kafka.kafka1.configId}")
    int configId;
    @Value(value = "${anms.task.kafka.kafka1.sleepTime}")
    int sleepTimp;
    @KafkaListener(topics = "#{'${anms.task.kafka.kafka1.topics}'.split(',')}", groupId = "${anms.task.kafka.kafka1.groupId}", autoStartup = "${anms.task.kafka.kafka1.isStart}")
    public void receive(ConsumerRecords<String, String> consumerRecords, Acknowledgment ack) {
        try {
            ...
            if (ObjectUtil.isNull(kafkaBatchClick) || ObjectUtil.isEmpty(kafkaBatchClick)) {
                ...
                log.info("kafka1-首次初始化数据库配置:" + kafkaBatchClick.toString());
            }
