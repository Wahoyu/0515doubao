...
public class Kafka4Listener {
    ...
    @Value(value = "${anms.task.kafka.kafka4.configId}")
    int configId;
    @Value(value = "${anms.task.kafka.kafka4.sleepTime}")
    int sleepTimp;

    @KafkaListener(topics = "#{'${anms.task.kafka.kafka4.topics}'.split(',')}", groupId = "${anms.task.kafka.kafka4.groupId}", autoStartup = "${anms.task.kafka.kafka4.isStart}")
    public void receive(ConsumerRecords<String, String> consumerRecords, Acknowledgment ack) {
        try {
            if (ObjectUtil.isNull(kafkaBatchClick) || ObjectUtil.isEmpty(kafkaBatchClick)) {
                ...
                log.info("kafka4-首次初始化数据库配置:" + kafkaBatchClick.toString());
            }
