...
public class diff_match_patch {
    ...
    public static Map getDeleteValue(List list, int num, String del_oper,
                                     String del_value) {
        ...
        for (int i = num - 1; i < list.size(); i--) {
            ...
            String oper = d.operation.toString();
            ...
            if (oper.equals("INSERT")) {
                ...
                value_map.put("value", value);
                value_map.put("point", "-1");
            ...
            if (oper.equals("DELETE")) {
                ...
                // ���?ĳ������ ǰ���һ��?Delete ���� ����ȡֵ���ڵ�λ��
                int point = getValueIndex(list, i);

//				System.out.println(oper + "---->" + value + "---->" + point);

                value_map.put("value", value);
                value_map.put("point", point);
    ...
    public static int getValueIndex(List list, int num) {
        int point = 0;
    ...
    public static Map<String, String> getKeyValue(String msg, String temp) {

        Map<String, String> stat_map = new LinkedHashMap<String, String>();

        if (NumUtil.isNUll(temp)) {
            return stat_map;
        }

        if (NumUtil.isNUll(msg)) {
            return stat_map;
        }

        if (temp.indexOf("${") > 0) {
            //按模板的开始字符串进行分割
            String split_str = temp.substring(0, temp.indexOf("${"));
            if (msg.indexOf(split_str) > 0) {
                msg = msg.substring(msg.indexOf(split_str));
            }
        }

        String msg_bak = msg.toLowerCase();

        String temp_bak = temp.toLowerCase();

        // 关键字转换为大写
        temp_bak = keyToUpperCase(temp_bak);

        diff_match_patch dmp = new diff_match_patch();

        List<Diff> list = dmp.diff_main(msg_bak, temp_bak);
        System.out.println(temp_bak + "-------" + list);
        boolean tag = false;
        for (int i = 0; i < list.size(); i++) {
            ...
            if (oper.equals("EQUAL") && tag) {
                stat_map.put(value, value);
            }
            ...
            if (oper.equals("INSERT")) {
                ...
                // System.out.println(oper + "---->" + value);

                // ��ȡ INSERT ֵ

                Map insert_value_map = getDeleteValue(list, i, oper, value);

                if (insert_value_map == null || insert_value_map.size() == 0) {
                    continue;
                }

                int point = Integer.parseInt(String.valueOf(insert_value_map
                        .get("point")));
                String insert_value = String.valueOf(insert_value_map
                        .get("value"));
                ...
                if (point >= 0) {
                    _value = msg
                            .substring(point, point + insert_value.length());
                }

                stat_map.put(value, _value);
        ...
        return getOriginalData(temp, stat_map);
    }

    public static String keyToUpperCase(String msg) {
        String[] s_array = msg.split("\\$\\{");
        String r_s = "";

        for (int i = 0; i < s_array.length; i++) {
            if (s_array[i].indexOf("}") > 0) {

                String key = s_array[i].substring(0, s_array[i].indexOf("}"))
                        .toUpperCase().replaceAll(" ", "<><><>")
                        + s_array[i].substring(s_array[i].indexOf("}"));
                r_s += "${" + key;
            } else {
                r_s += s_array[i];
            }
    ...
    public static Map<String, String> mergeData(Map<String, String> data) {

        Map<String, String> r_data = new LinkedHashMap<String, String>();
...
//					+ entry.getValue());
            ...
            String key = key_temp + entry.getKey();
            String value = value_temp + entry.getValue();

            if (key.indexOf("${") >= 0 && key.indexOf("}") >= 0) {

                r_data.put(key, value);
    ...
    public static Map<String, String> getOriginalData(String msg,
        ...
        for (int i = 0; i < s_array.length; i++) {
            if (s_array[i].indexOf("}") > 0) {
                ...
                if (data.containsKey(key_temp)) {

                    String value = data.get(key_temp);

                    if (value != null) {
                        value = value.trim();
                    }
                    // 删除已经有的数据
                    data.remove(key_temp);
    ...
    //		data:gpon-onu_1/2/1:3 enable enable operation working
    //		temp:gpon-onu_1/2/1:3 ${ADMIN} ${OMCC} ${OSEVEN} ${PHASESTATE}
            String msg = "<186>Aug 14 2014 12:41:41+08:00 HLZX-HR5KE-MB02 %%01STANDARD/2/linkDown_clear(l):VS=Admin-VS-CID=0x807a271a-alarmID=0x08520003-clearType=service_resume;The interface status changes. (ifName=GigabitEthernet6/0/23, AdminStatus=UP, OperStatus=UP, Reason=The link protocol is Up.)";
            while(msg.indexOf("  ")>=0){
                msg = msg.replaceAll("  ", " ");
            }
            String temp = "The interface status changes. (ifName=${OBJ_NAME}, AdminStatus=";
            Map<String, String> sssa = diff_match_patch.getKeyValue(msg,temp);
            System.out.println("*********\n"+sssa);
            String text = "<186>Aug 14 2014 12:41:41+08:00 HLZX-HR5KE-MB02 %%01STANDARD/2/linkDown_clear(l):VS=Admin-VS-CID=0x807a271a-alarmID=0x08520003-clearType=service_resume;The interface status changes. (ifName=GigabitEthernet6/0/23, AdminStatus=UP, OperStatus=UP, Reason=The link protocol is Up.)";
            int type = 0;
